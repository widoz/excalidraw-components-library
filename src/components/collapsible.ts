...
export default function collapsible(theme: Theme): ComponentOutput {
  const f = new Factory("collapsible", theme);

  // Expanded state.
  const expandedEls: ExcalidrawElement[] = [];
  expandedEls.push(...inkBox(f, { x: 0, y: 0, w: W, h: TRIGGER_H }));
  expandedEls.push(...label(f, {
    x: 24,
  ...
  collapsedEls.push(...inkBox(f, { x: 0, y: trigger2Y, w: W, h: TRIGGER_H }));
  collapsedEls.push(...label(f, {
    x: 24,
